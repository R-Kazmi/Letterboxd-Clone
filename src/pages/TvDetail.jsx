...
const TvDetail = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      const request = await axios.get(`/tv/${tvId}?api_key=2750142d8e261787e7239ae8d1758d35&language=en-US`);
      const request2 = await axios.get(`/tv/${tvId}/credits?api_key=2750142d8e261787e7239ae8d1758d35&language=en-US`);
      setTv(request.data);
      ...
      {tv &&
        <>
          <div id='backdrop-div' className='flex items-center justify-center'>
            <img className='opacity-25 object-cover' src={`${base_url}${tv.backdrop_path}`} alt="" />
          </div>
          ...
          <div className='flex justify-center items-center flex-col' id='movie-detail-container'>
            <div id='movie-detail-container-flex1' className='flex relative bottom-24'>
              <div className='relative left-8' id='movie-detail-container-flex1-div1'>
                <img className='sticky top-5 rounded-lg' src={`${base_url}${tv.poster_path}`} alt="" />
              </div>
              <div id='movie-detail-container-flex1-div2' className='flex flex-col ml-4 relative left-8'>
                <div className='flex items-center'>

                  <h1 className='text-3xl text-white ml-2'>{tv.original_name}</h1>
                  <h3 className='text-base'>{tv?.first_air_date?.slice(0, 4)}</h3>
                </div>
                <div className='mt-4' id='movie-detail-container-flex1-div2-tagline'>
                  <p className='text-sm'>{tv?.tagline}</p>
                </div>
                <div className='mt-4' id='movie-detail-container-flex1-div2-overview'>
                  <p>{tv.overview}</p>
                </div>
              ...
              <div className='flex flex-col rounded-md ml-2 relative top-14 left-10 h-64' id='movie-detail-container-flex1-div3'>
